...
for (i in 1:length(dataset_names)) {
  ...
  for (Q in c("Q1", "Q3")) {
    thisPeriodicTable <- read.delim(
      paste0(dirs[i], "/", Q, "_periodic_proportions.txt"),
      header = FALSE,
      sep = "\t"
    )
    periodic_E <- c(
    ...
    periodic <- rbind(
      ...
      data.frame(
        ...
        quartile = rep(Q, nrow(thisPeriodicTable))
      )
    )
    
    thisAntiPeriodicTable <- read.delim(
      paste0(dirs[i], "/", Q, "_anti_periodic_proportions.txt"),
      header = FALSE,
      sep = "\t"
    )
    anti_periodic_E <- c(
    ...
    anti_periodic <- rbind(
      ...
      data.frame(
        ...
        quartile = rep(Q, nrow(thisAntiPeriodicTable))
      )
    )
...
rm(thisFreqTable, thisPeriodicTable, thisAntiPeriodicTable, N, i, j, k, Q)
    ...
    for (Q in c("Q1", "Q3")) {
      thisDataSubset <- distance_frequencies[
        (distance_frequencies$putative == "Weiner et al.") &
          ...
          (distance_frequencies$quartile == Q),
      ]
      ...
      weinerPoints <- rbind(
        weinerPoints,
        data.frame(mean=M, std=S, n=N, reference=reference_names[j], quartile=Q)
      )
...
for (i in 1:length(dataset_names)) {
  ...
  for (Q in c("Q1", "Q3")) {
    if (dataset_names[i] != "Weiner et al.")
    {
      ...
      for (j in 1:length(reference_names)) {
        ...
        if (reference_names[j] != "Weiner et al.")
        {
          ...
          ggsave(
            paste0(dirs[i],"/",reference_datasets[j],"_",Q,"_distances.png"),
            width = 2000,
            height = 1000,
            units = "px"
          )
      ...
      ggsave(
        paste0(dirs[i], "/", Q, "_mean_distances.png"),
        width = 2000,
        height = 1000,
        units = "px"
      )
    ...
    if (dataset_names[i] != "Weiner et al.") {
      ggplot(
        data = periodic[
          (periodic$putative == dataset_names[i] |
            periodic$putative == "Weiner et al.") & periodic$quartile == Q,
        ]
      ) +
        geom_col(
      ...
      ggsave(
        paste0(dirs[i], "/", Q, "_periodic_proportion_barplot.png"),
        width = 2000,
        height = 1000,
        units = "px"
      )
      ...
      ggsave(
        paste0(dirs[i], "/", Q, "_anti_periodic_proportion_barplot.png"),
        width = 2000,
        height = 1000,
        units = "px"
      )
