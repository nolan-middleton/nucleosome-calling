...
for (i in 1:length(dirs)) {
  data <- read_json(paste0(dirs[i], "/rotational_categories_lenient.json"))
...
for (i in 1:length(random_dirs)) {
  ...
  data<-read_json(paste0(random_dirs[i],"/rotational_categories_lenient.json"))
...
ggsave(
  paste0("rotational_categories_histogram_lenient.png"),
  width = 2500,
  height = 1250,
  units = "px"
)
